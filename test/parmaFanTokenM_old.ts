...
let parmaFanTokenM_old: Contract;
...
describe("Testing Parma fan token M...", async () => {
    ...
    describe("Setup contract", async () => {
        ...
        it("Should deploy parma fan token M", async () => {
            let factory = await ethers.getContractFactory("ParmaFanTokenM_old")
            parmaFanTokenM_old = await factory.deploy(nameParmaFanTokenM, symbolParmaFanToken, parmaFanToken.address)
            expect(await parmaFanTokenM_old.parmaFanTokenAddress()).to.be.equal(parmaFanToken.address);
        })
    ...
    describe("Fund contract", async () => {
        ...
        it("Fund Parma token M with Parma token", async () => {
            ...
            let tx = await parmaFanToken.connect(deployer).transfer(parmaFanTokenM_old.address, amountToFound);
            await tx.wait();
            expect(await parmaFanToken.balanceOf(parmaFanTokenM_old.address)).to.be.equal(amountToFound);
        })
    ...
    describe("Lock", async () => {
        ...
        it("Should lock token with vesting type 1", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            let tx = await parmaFanTokenM_old.deliverToAccount(account1.address, amountLockType1, 1)
            await tx.wait();
            let amountTokenMDeliverExpected = amountLockType1.mul(85).div(100);
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore.add(amountTokenMDeliverExpected));
            expect(await parmaFanTokenM_old.balanceOf(account1.address)).to.be.equal(amountTokenMDeliverExpected)
        })

        it("Should lock token with vesting type 2", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            let tx = await parmaFanTokenM_old.deliverToAccount(account2.address, amountLockType2, 2)
            await tx.wait();
            let amountTokenMDeliverExpected = amountLockType2.mul(80).div(100);
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore.add(amountTokenMDeliverExpected));
            expect(await parmaFanTokenM_old.balanceOf(account2.address)).to.be.equal(amountTokenMDeliverExpected)
        })

        it("Should lock token with vesting type 3", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            let tx = await parmaFanTokenM_old.deliverToAccount(account3.address, amountLockType3, 3)
            await tx.wait();
            let amountTokenMDeliverExpected = amountLockType3;
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore.add(amountTokenMDeliverExpected));
            expect(await parmaFanTokenM_old.balanceOf(account3.address)).to.be.equal(amountTokenMDeliverExpected)
        })

        it("Should lock token with vesting type 4", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            let tx = await parmaFanTokenM_old.deliverToAccount(account4.address, amountLockType4, 4)
            await tx.wait();
            let amountTokenMDeliverExpected = amountLockType4;
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore.add(amountTokenMDeliverExpected));
            expect(await parmaFanTokenM_old.balanceOf(account4.address)).to.be.equal(amountTokenMDeliverExpected)
        })

        it("Should revert lock token with vesting type 1", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            await catchRevert(parmaFanTokenM_old.deliverToAccount(account1.address, amountLockType1, 1))
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore);
        })

        it("Should revert lock token with vesting type 2", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            await catchRevert(parmaFanTokenM_old.deliverToAccount(account2.address, amountLockType2, 2))
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore);
        })

        it("Should revert lock token with vesting type 3", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            await catchRevert(parmaFanTokenM_old.deliverToAccount(account3.address, amountLockType3, 3))
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore);
        })

        it("Should revert lock token with vesting type 4", async () => {
            let totalSupplyBefore = await parmaFanTokenM_old.totalSupply();
            await catchRevert(parmaFanTokenM_old.deliverToAccount(account4.address, amountLockType4, 4))
            expect(await parmaFanTokenM_old.totalSupply()).to.be.equal(totalSupplyBefore);
        })
    ...
    describe("Unlock token", async () => {
        ...
        it("Unlock all type 1, 2, 3", async () => {
            for (let i = 0; i < 25; i++) {
                ...
                if (i > 1 && i < 19) {
                    ...
                    let tx = await parmaFanTokenM_old.connect(account1).swapTokenUnlock();
                    await tx.wait();
                ...
                if (i > 0 && i < 8) {
                    ...
                    let tx = await parmaFanTokenM_old.connect(account2).swapTokenUnlock();
                    await tx.wait();
                ...
                if (i > 4) {
                    ...
                    let tx = await parmaFanTokenM_old.connect(account3).swapTokenUnlock();
                    await tx.wait();
        ...
        it("Unlock type 4", async () => {
            let tx = await parmaFanTokenM_old.unlockSwap(account4.address);
            await tx.wait();

            let balanceBeforeSwap = await parmaFanToken.balanceOf(account4.address);
            tx = await parmaFanTokenM_old.connect(account4).swapTokenUnlock();
            await tx.wait();
        ...
        it("Revert unlock type 1", async () => {
            let balanceBeforeSwap = await parmaFanToken.balanceOf(account1.address);
            await catchRevert(parmaFanTokenM_old.connect(account1).swapTokenUnlock());
        ...
        it("Revert unlock type 2", async () => {
            let balanceBeforeSwap = await parmaFanToken.balanceOf(account2.address);
            await catchRevert(parmaFanTokenM_old.connect(account2).swapTokenUnlock());
        ...
        it("Revert unlock type 3", async () => {
            let balanceBeforeSwap = await parmaFanToken.balanceOf(account3.address);
            await catchRevert(parmaFanTokenM_old.connect(account3).swapTokenUnlock());

            expect(await parmaFanToken.balanceOf(account3.address)).to.be.equal(balanceBeforeSwap)
        })

        it("Revert unlock type 4", async () => {
            await catchRevert(parmaFanTokenM_old.unlockSwap(account4.address));

            let balanceBeforeSwap = await parmaFanToken.balanceOf(account4.address);
            await catchRevert(parmaFanTokenM_old.connect(account4).swapTokenUnlock());
